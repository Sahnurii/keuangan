...
// Cek data awal dari tabel 'kampus'
$sql_cek = "SELECT * FROM kampus WHERE id_kampus";
$query_cek = mysqli_query($koneksi, $sql_cek);
$data_cek = mysqli_fetch_array($query_cek, MYSQLI_BOTH);

// Tentukan jalur gambar, gunakan default 'noimage.png' jika tidak ada gambar
$gambar = $data_cek['gambar_kampus'] ? "admin/gambar/" . $data_cek['gambar_kampus'] : "admin/gambar/noimage.png";
?>

<?php

if (isset($_POST['Ubah'])) {
    // Validasi apakah folder tujuan tersedia dan dapat ditulis
    $target_dir = __DIR__ . "/admin/gambar/";
    if (!is_dir($target_dir) || !is_writable($target_dir)) {
        $error = "Folder tujuan tidak ditemukan atau tidak memiliki izin tulis.";
    } else {
        ...
        if ($fileName) {
            ...
            if (!in_array($fileType, $allowedTypes)) {
                $error = "Jenis file yang diunggah tidak diizinkan. Hanya JPG, JPEG, PNG, dan GIF yang diperbolehkan.";
                $uploadOk = false;
            }

            // Batas ukuran file (2MB)
            if ($_FILES["gambar_kampus"]["size"] > 2000000) {
            ...
            if ($uploadOk) {
                ...
                if (move_uploaded_file($_FILES['gambar_kampus']['tmp_name'], $target_file)) {
                    // Perbarui database setelah unggah berhasil
                    ...
                    $error = "Gagal mengunggah file. Pastikan file valid dan folder tujuan tersedia.";
                }
            }
        } else {
