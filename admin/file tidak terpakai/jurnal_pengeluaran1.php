...
// Ambil saldo terakhir berdasarkan pilihan lokasi data
if (isset($_POST['get_saldo'])) {
    $lokasi_data = $_POST['lokasi_data'];
    if ($lokasi_data == "Buku Pembantu Kas") {
        $query = "SELECT saldo FROM b_pembantu_kas ORDER BY id_pembantu_kas DESC LIMIT 1";
    } else if ($lokasi_data == "Buku Pembantu Bank") {
        $query = "SELECT saldo FROM b_pembantu_bank ORDER BY id_pembantu_bank DESC LIMIT 1";
    }
    $result = mysqli_query($koneksi, $query);
    $saldo_terakhir = mysqli_fetch_assoc($result)['saldo'];
    echo json_encode($saldo_terakhir); // Kembalikan saldo terakhir dalam bentuk JSON
    exit;
}
            ...
            <form action="" method="POST" id="form-jurnal">
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="number" step="0.01" class="form-control" id="pengeluaran_kredit" name="pengeluaran_kredit">
                        </div>
                    </div>
                </div>
